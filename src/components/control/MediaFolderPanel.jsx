...
export function MediaFolderPanel() {
  ...
  const open = () => canUseFileSystemPicker() ? run(() => runtime.commands.addMediaFiles()) : input.current?.click();
  return <section className="media-folder-panel" aria-label="Media files" onKeyDown={e => e.stopPropagation()}>
    <FolderControls label="Media" folder={status.folder} permission={status.permission} busy={busy} run={run}
      ...
      note="Scans images and videos in this folder (not subfolders). Refresh adds new files. Unlink stops scanning; existing media references and source files are kept.">
      <button className="library-add-btn media-add-btn" aria-label="Add media" disabled={busy} onClick={open}>ADD</button>
      <IconControl icon="open" label="Open Media" title="Open individual image or video files" disabled={busy} onClick={open} />
    </FolderControls>
