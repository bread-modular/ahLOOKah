// Legacy built-in adapter: the pre-refactor centralized declarations, extracted
// verbatim from src/sketch-registry.js. Temporary home for OLD modules only —
// new patterns use self-describing `src/sketches/<id>.pattern.js` modules and
// automatic discovery. Old modules may migrate there over time; adding a new
// pattern must never touch this file.
//
// NOTE: the Simple Circle descriptor was removed from this adapter when the
// pattern migrated to the new convention (src/sketches/simple-circle.pattern.js).
import circles, {
...
// Simple Circle migrated to src/sketches/simple-circle.pattern.js (auto-discovered).
...
// Expansion wave: 9 researched VJ/VFX patterns and two restored legacy
// camera looks.
...
export const LEGACY_BUILTINS = [
  ...
  // NOTE: no new entries here — Simple Circle was the last manual addition and
  // now lives at src/sketches/simple-circle.pattern.js (auto-discovered).
];
