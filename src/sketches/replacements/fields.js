...
void main() {
  ...
  if (hash21(cell) < .25 + uMid * 1.1) q.x = 1.0 - q.x;
  float d = min(abs(length(q) - .5), abs(length(q - 1.0) - .5));
  float outer = line(d, .04 + uSub * .22);
  float inner = line(d - .03 - uHigh * .32, .018);
...
void main() {
  ...
  p *= 1.0 + uSub * .9 * exp(-length(p) * .7);
  p = rotate2d(uMid * 1.5) * p;
  p.x += uMid * .7 * sin(p.y * 2.5 + t);
  float x = p.x * 3.0 * uDetail;
  float y = p.y * 3.0 * uDetail;
  float f = cos(x + .12 * sin(t)) * cos(y) - cos(y * 1.6) * cos(x * .6);
  f += uHigh * 1.5 * sin(p.x * 21.0 + p.y * 13.0 + t * 3.0);
...
void main() {
  ...
  p += (warp - .5) * (1.1 + uSub * 6.5);
  p = rotate2d(uMid * (2.2 + length(p) * 1.2)) * p;
  float f = fbm4(p * (2.0 * uDetail) + vec2(t, -t));
  float contour = sin(f * (24.0 + uHigh * 85.0) + p.y * 2.0);
...
float glass(vec3 p) {
  ...
  p.xy = rotate2d(t + uMid * 2.6) * p.xy;
  float a = length(p - vec3(-.3 - uSub * .85, .05, 0.0)) - (.43 + uSub * .12);
  float b = length(p - vec3(.4 + uSub * .3, .12 + uMid * .85, .05)) - .48;
  float c = length(p - vec3(.05, -.42, .1)) - .34;
  return softUnion(softUnion(a, b), c) + sin(p.y * (18.0 * uDetail) + t * 3.0) * sin(p.x * 13.0) * uHigh * .115;
}
void main() {
  vec2 uv = vTexCoord * 2.0 - 1.0; uv.x *= uResolution.x / uResolution.y;
...
void main() {
  ...
  vec2 address = floor(uv * (48.0 * uDetail / (1.0 + uSub * 2.6)));
  float x = mod(address.x + floor(uMid * 97.0 * sin(address.y * .19 + uTime * uSpeed)) + t, 256.0);
  float y = mod(address.y * (2.0 + floor(uHigh * 23.0)), 256.0);
...
void main() {
  ...
  float steps = max(2.0, 8.0 * uDetail / (1.0 + uHigh * 3.2));
  ...
  float shift = (uSub - .2) * 2.3 / (1.0 + uSub * 1.2);
...
void main() {
  ...
  for (int y = -1; y <= 1; y++) for (int x = -1; x <= 1; x++) {
    ...
    vec2 site = .5 + (.14 + uMid * .72) * sin(vec2(hash21(id), hash21(id + 8.0)) * 6.28 + uTime * uSpeed * .65);
  ...
  float wall = 1.0 - smoothstep(.025 + uSub * .38, .055 + uSub * .38, second - first);
  float holes = smoothstep(.09, .12, first);
  holes *= smoothstep(uHigh * .95 - .04, uHigh * .95 + .02, abs(sin(p.x * 7.0 + p.y * 5.0 + uTime * uSpeed * 2.0)));
  ...
  float vents = line(first - (.11 + uHigh * .62), .012 + uHigh * .035) * smoothstep(0.0, .08, uHigh);
...
export const FIELD_PATTERNS = [
  ...
  entry('membrane-modes', 'Membrane Modes', 'Rhythmic', 'Standing-wave nodal sand: bass bows the membrane, mids shear its nodal lattice, highs fracture fine lines.', membrane, 'Mode Scale'),
  ...
  entry('tidal-glass', 'Tidal Glass', 'Cinematic / Shaders', 'Raymarched glass lobes: bass separates bodies, mids lift and turn the upper lobe, highs corrugate the glass surface.', tidalGlass, 'Surface Frequency'),
  entry('bitplane-rewire', 'Bitplane Rewire', 'Glitch / Effects', 'XOR address-plane corruption: bass grows address blocks, mids displace scanline addresses, highs rewire row strides.', bitplane, 'Address Density'),
  entry('stair-wipe', 'Stair Wipe', 'Basics', 'Stepped diagonal wipe: bass translates the reveal, mids turn its axis, highs unfold deeper staircase teeth.', stairWipe, 'Step Count'),
