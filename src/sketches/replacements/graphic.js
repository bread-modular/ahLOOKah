...
const counterweight = canvasFactory((ctx, { t, b, m, h, kick, snare, hat, hue }) => {
  ...
  // Kick punches the swing, snare snaps the joints, hats flash the jaw tips.
  for (let i = 0; i < 5; i++) {
    const x = (i - 2) * .55, a = Math.sin(t + i * .72) * .12 + (i % 2 ? -1 : 1) * (b * 1.65 + kick * .5);
    const len = .47 + b * .3 + kick * .08, joint = [x + Math.sin(a) * len, -.65 + Math.cos(a) * len];
    const a2 = -a + (m * 2.5 + snare * 1.1) * Math.sin(i * 1.2 + .5 + t * .7);
    const end = [joint[0] + Math.sin(a2) * .45, joint[1] + Math.cos(a2) * .45];
    path(ctx, [[x, -.8], [x, -.65], joint, end], null, color(hue, 70, 20), .035);
    circle(ctx, ...joint, .065 + b * .03 + kick * .02, color(hue + .08));
    circle(ctx, x, -.8, .035, '#fff');
    for (const side of [-1, 1]) {
      ctx.save(); ctx.translate(...end); ctx.rotate(side * (.12 + h * 2.2 + hat * .45));
      path(ctx, [[0, -.08], [side * .13, -.08], [side * (.17 + h * .1), .19 + h * .24], [0, .19 + h * .24]], color(hue + i * .06), '#0a1020');
      ctx.restore();
      if (hat > .01) {
        ctx.save(); ctx.globalAlpha = hat;
        circle(ctx, end[0] + side * .1, end[1] + .12, .01 + hat * .05, color(hue + .5, 85));
        ctx.restore();
      }
    }
  }
...
const ratchet = canvasFactory((ctx, { t, b, m, h, kick, snare, hat, beat, detail, hue }) => {
  const teeth = Math.round(12 * detail), radius = .40 + b * .4 + kick * .09;
  ctx.save(); ctx.rotate(t * .45 + m * 2.8 + snare * .45);
  const points = [];
  for (let i = 0; i < teeth * 4; i++) {
    const a = i / (teeth * 4) * TAU, r = radius + (i % 4 < 2 ? .12 + h * .3 + hat * .06 : 0);
  ...
  circle(ctx, 0, 0, .14 * (1 + beat * .25), color(hue + .5)); ctx.restore();
  // A visible escapement pawl and reciprocal striker make a coherent machine.
  for (const side of [-1, 1]) {
    const x = side * (1.3 - b * .36 - kick * .1);
    path(ctx, [[x, -.6], [x - side * (.28 + m * .4 + snare * .12), -.13], [x, .08]], color(hue + .5, 70), '#fff');
    path(ctx, [[x, .25], [x, .75]], null, color(hue, 45), .04);
    ctx.fillStyle = color(hue + .5); ctx.fillRect(x - .12, .24 + h * .38, .24, .2);
    if (hat > .01) { ctx.save(); ctx.globalAlpha = hat; ctx.fillStyle = color(hue + .5, 85); ctx.fillRect(x - .12, .24 + h * .38, .24, .03 + hat * .05); ctx.restore(); }
  }
});

const vectorKnot = canvasFactory((ctx, { t, b, m, h, kick, snare, hat, detail, hue }) => {
  const pts = [];
  for (let i = 0; i <= 420; i++) {
    const a = i / 420 * TAU, r = .58 + (.1 + b * .5 + kick * .22) * Math.cos(3 * a + t);
    const x = r * Math.cos(2 * a), y = r * Math.sin(2 * a), z = (.14 + m * .72) * Math.sin(3 * a + t);
    const tilt = .6 + m * 1.65 + snare * .3, py = y * Math.cos(tilt) - z * Math.sin(tilt);
  ...
  for (const offset of [-1, 0, 1]) {
    const track = pts.map(([x, y], i) => {
      ...
      const spread = offset * (.008 + (h + hat * .3) * (.18 + .06 * Math.sin(i * .12 - t * 3)));
      return [x - dy / len * spread, y + dx / len * spread];
    });
    path(ctx, track, null, color(hue + offset * .18, 25), .055 * detail * (1 + kick * .3));
    path(ctx, track, null, color(hue + offset * .18, 76), .013 * detail * (1 + hat * .8));
  }
  if (hat > .01) {
    ctx.save(); ctx.globalAlpha = hat;
    for (let i = 0; i < pts.length; i += 35) circle(ctx, pts[i][0], pts[i][1], .012 + hat * .05, color(hue + .5, 88));
    ctx.restore();
  }
});

const prismScanner = canvasFactory((ctx, { t, b, m, h, kick, snare, hat, beat, hue }) => {
  const triangle = [[-.6, .62], [.0, -.65], [.65, .62], [-.6, .62]];
  path(ctx, triangle, color(hue, 9 + beat * 8, 35), color(hue, 70), .025);
  const launch = [-1.65, -.5 + Math.sin(t) * .08 + b * 1.05 + kick * .22];
  const strike = [-.4 + m * .9 + snare * .22, -.15 + b * .45 + kick * .12];
  path(ctx, [launch, strike], null, '#eaffff', .025 * (1 + kick * 1.2));
  for (let i = 0; i < 9; i++) {
    const angle = -.9 - h * .8 + i * (.1 + h * .27 + hat * .05) + m * .7;
    const end = [1.65, strike[1] + Math.sin(angle) * 1.4];
    path(ctx, [strike, [.52, .4 - b * .45], end], null, color(hue + i * .055, 24), .065);
    path(ctx, [strike, [.52, .4 - b * .45], end], null, color(hue + i * .055, 72), .014 * (1 + hat * .9));
  }
  circle(ctx, ...strike, .055 + kick * .04 + snare * .03, '#fff');
  if (snare > .01) {
    ctx.save(); ctx.globalAlpha = snare;
    circle(ctx, ...strike, .12 + snare * .3, null, color(hue + .5, 85), .012);
    ctx.restore();
  }
});
...
const barnDoors = canvasFactory((ctx, { t, b, m, h, kick, snare, hat, detail, hue }) => {
  ctx.fillStyle = '#000'; ctx.fillRect(-4, -1, 8, 2);
  ctx.save(); ctx.rotate(m * 1.8 + snare * .35 + Math.sin(t * .4) * .025);
  ctx.translate(m * .4, 0);
  const x = .22 + b * .95 + kick * .18, y = .68;
  ctx.fillStyle = color(hue, 78, 35);
  // Highs articulate FIVE separate shutters, not another global scale axis.
  for (let i = 0; i < 5; i++) {
    const row = 2 * y / 5, shift = (i % 2 ? -1 : 1) * (h * .32 + hat * .1);
    ctx.fillRect(-x * detail + shift, -y + i * row + h * .075 + hat * .02, 2 * x * detail, row - h * .15 - hat * .04);
  }
...
export const GRAPHIC_PATTERNS = [
  entry('counterweight', 'Counterweight', 'Simple', 'Coupled hanging levers: bass swings the arms, mids articulate joints, highs open the jaws; kicks punch the swing, snares snap the joints, hats flash the jaw tips.', counterweight, null),
  entry('ratchet-wheel', 'Ratchet Wheel', 'Rhythmic', 'Mechanical escapement: bass expands the gear, mids advance the ratchet, highs extend teeth and strikers; kicks punch the gear, snares snap the advance, hats flash the strikers.', ratchet, 'Tooth Count', { accents: true }),
  entry('vector-knot', 'Vector Knot', 'Neon / Lasers', 'Laser torus knot: bass changes knot lobes, mids tilt depth, highs separate the three beam tracks; kicks deepen lobes, snares snap the tilt, hats spark the vertices.', vectorKnot, 'Beam Width', { accents: true }),
  entry('prism-scanner', 'Prism Scanner', 'Neon / Lasers', 'Prismatic scanner: bass moves the incident beam, mids move its strike point, highs fan the refracted paths; kicks flare the beam, snares ring the strike, hats brighten the fan.', prismScanner, null, { accents: true }),
  ...
  entry('barn-doors', 'Barn Doors', 'Basics', 'Stage shutter aperture: bass opens width, mids rotate the doors, highs separate five sliding louvers; kicks punch the aperture, snares snap the hinge, hats rattle the louvers.', barnDoors, 'Aperture Aspect'),
