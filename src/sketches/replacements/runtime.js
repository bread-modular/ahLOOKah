...
// Audio consumption copies the three reference implementations (Techno 3D,
// Circles, Ion Tempest) via the shared feature-controls transport: sustained
// band levels PLUS independent gated percussion/energy channels, direct
// mappings, no fabricated idle beats.
...
export function entry(id, name, group, description, factory, detailLabel, extra = {}) {
  const { accents = false, ...rest } = extra;
  const spec = reactiveEntry({ id, name, group, description, factory, ...rest, params: [
  ...
  return { ...spec, audioControlSchema: FEATURE_SCHEMA, createAudioController: createReplacementController };
}
