...
test('actual Web Audio capture → analyser → noise floor → engine → packet/store → all 11 expansion renderers; real alpha compositing', { tag: '@patterns' }, async ({ page }) => {
  ...
  // 48 accepted frames × the 9 retained expansion-cohort patterns
  // (7 retained + 2 restored; pinned by expansion-inventory.spec.js).
  expect(result.accepted).toBe(48 * 9);
