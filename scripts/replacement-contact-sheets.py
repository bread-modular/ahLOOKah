...
ids = 'truchet-relay membrane-modes schlieren-flow tidal-glass bitplane-rewire riso-misprint iris-diaphragm cellular-gate'.split()
...
for page in range(2):
    ...
    links.append(f'<h2>Patterns {page * 6 + 1}–{page * 6 + len(subset)}</h2><p><a href="{gif}">Animated silence / pulsed audio / diff (1.2 s)</a></p><a href="{filename}"><img src="{filename}" width="1280" alt="Six labeled A/B/strong/difference comparisons"></a>')
